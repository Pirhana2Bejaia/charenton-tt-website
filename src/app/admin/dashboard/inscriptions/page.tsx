...
export default function AdminInscriptions() {
  ...
  const [formData, setFormData] = useState<DocumentFormData>({
    title: '', file_url: '', type: 'inscription'
  });
  ...
  if (isEditing) {
    return (
      <div>
        <h2 className="text-2xl font-bold mb-6">Nouveau Formulaire</h2>
        <form onSubmit={handleSubmit} className="admin-card space-y-6 max-w-2xl">
          <div>
            <label className="block text-sm font-medium mb-2">Titre (ex: Formulaire Adulte 2026-2027)</label>
            <input required type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="admin-input" />
          </div>
          <div>
            <label className="block text-sm font-medium mb-2">Fichier (Image du formulaire)</label>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        ...
        <button onClick={() => { setFormData({title:'', file_url:'', type:'inscription'}); setIsEditing(true); }} className="admin-btn admin-btn-primary"><Plus size={18} /> Ajouter</button>
      </div>
